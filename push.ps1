# ---------------------------------------------------------------
#  JellyFusion v2.0.6  -  Force push + auto-verify
#
...
git commit -m "JellyFusion v2.0.6 - BadgeMiddleware IStartupFilter, Math.Max badge fix, ParseColor hardening, Theme/Slider cleanups, custom badge labels" | Out-Null
...
git tag -f -a $tag -m "JellyFusion v2.0.6 - all-in-one plugin"
...
try {
    ...
    if ($versions -contains '2.0.6.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v2.0.6 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusion/releases/tag/v2.0.6'
} else {
    Write-Warning 'Remote manifest does not yet show v2.0.6.0 - raw cache is still warm.'
