# ---------------------------------------------------------------
#  JellyFusion v2.0.5  -  Force push + auto-verify
#
...
git commit -m "JellyFusion v2.0.5 - robust config load, per-panel try/catch, dirty indicator on Save" | Out-Null
...
git tag -f -a $tag -m "JellyFusion v2.0.5 - all-in-one plugin"
...
try {
    ...
    if ($versions -contains '2.0.5.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v2.0.5 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusion/releases/tag/v2.0.5'
} else {
    Write-Warning 'Remote manifest does not yet show v2.0.5.0 - raw cache is still warm.'
