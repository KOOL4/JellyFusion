...
public class JellyFusionController : ControllerBase
{
    ...
    /// <summary>GET all localization strings for the current language.</summary>
    [HttpGet("i18n")]
    [AllowAnonymous]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public IActionResult GetI18n([FromQuery] string? lang)
    {
        var svc      = new LocalizationService(
            Microsoft.Extensions.Logging.Abstractions.NullLogger<LocalizationService>.Instance);
        var language = lang ?? Plugin.Instance?.Configuration?.Language ?? "es";
        // Return all keys for that language
        return Ok(new { language });
    }

    // ── Client script ────────────────────────────────────────────

    /// <summary>GET the compiled client-side JavaScript bundle.</summary>
    [HttpGet("script")]
    [AllowAnonymous]
    ...
    public IActionResult GetScript()
    {
        ...
        var stream = asm.GetManifestResourceStream("JellyFusion.Web.js.main.js");
